...
//    { .type = PERF_TYPE_HW_CACHE, .config = PERF_COUNT_HW_CACHE_DTLB |(PERF_COUNT_HW_CACHE_OP_READ << 8)|(PERF_COUNT_HW_CACHE_OP_WRITE << 8)| (PERF_COUNT_HW_CACHE_RESULT_ACCESS << 16) },
//    { .type = PERF_TYPE_HW_CACHE, .config = PERF_COUNT_HW_CACHE_DTLB |(PERF_COUNT_HW_CACHE_OP_READ << 8)| (PERF_COUNT_HW_CACHE_OP_WRITE << 8)| (PERF_COUNT_HW_CACHE_RESULT_MISS << 16) },
...
void restore_cpufrequnecy() {
    char cmdline[1024];
    sprintf(cmdline,"/usr/sbin/changefreq %d 800000 4600000",cpu);
    system(cmdline);
}
...
void perfstats_disable(unsigned long long *garbage)
{
    int i;
...
//    prefetch_misses = readcounter(DL1_PREFETCH_MISSES);
    ...
    fprintf(fout, "%s%llu,%llu,%lf,%lf,%lf,%lf,%llu,%llu%s", 
                     ...
                     ((double)elapsed_time*1000000000.0)/cycles, 
                      elapsed_time,(double)(load_misses+store_misses+prefetch_misses)/(double)(loads+stores+prefetches),load_misses+store_misses+prefetch_misses,loads+stores+prefetches,epilogue);
/*    for (i = 0; i < STAT_COUNT; i++) {
        printf("%lld\n",performance_counters[i]);
    }
*/
/*                      
    fprintf(fout, "%s%llu,%llu,%lf,%lf,%lf,%lf,%llu,%llu%s", 
                     ...
                     (double)cycles/ic,
                     ((double)elapsed_time*1000000000.0)/cycles, 
                      elapsed_time,(double)(load_misses+store_misses)/(double)(loads+stores+prefetches),load_misses+store_misses,loads+stores,epilogue);
*/    fclose(fout);
}
#ifdef __cplusplus
}
